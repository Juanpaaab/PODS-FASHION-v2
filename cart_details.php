	...
	if(isset($_SESSION['user'])){
		if(isset($_SESSION['cart'])){
			foreach($_SESSION['cart'] as $row){
				...
				$stmt->execute(['user_id'=>$user['id_user'], 'product_id'=>$row['productid']]);
				$crow = $stmt->fetch();
				if($crow['numrows'] < 1){
					$stmt = $conn->prepare("INSERT INTO cart (user_id, product_id, quantity) VALUES (:user_id, :product_id, :quantity)");
					$stmt->execute(['user_id'=>$user['id_user'], 'product_id'=>$row['productid'], 'quantity'=>$row['quantity']]);
				}
				else{
		...
		try{
			...
			$stmt->execute(['user'=>$user['id_user']]);
			foreach($stmt as $row){
